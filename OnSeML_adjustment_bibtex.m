...
para.filename = 'bibtex'; %Corel5k, bibtex delicious mediamill
...
para.basize = 2000;
para.qsize = 50;
para.sq = round(para.qsize*1.1);

para.tk = 2; % number of predicted labels
...
for i = 2:num_data 
    ...
    if i > size_ini && label_index(i) == 1
        ...
        if count == para.qsize
            ...
            tH_new = tP' * dataY(:, temp_label_budget);            
            tQ_new = dataY(:, temp_label_budget) * tH_new' * inv(tH_new * tH_new');
